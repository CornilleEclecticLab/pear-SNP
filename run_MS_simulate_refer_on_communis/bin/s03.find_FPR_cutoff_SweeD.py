...
# @File     : s03.find_FPR_cutoff_SweeD.py
# @Author   : NIE Yuqi
...
s02_folder = os.path.join(
    output_dir, 's02.generate_positive_selection_detect_SweeD')
...
for pop in populations.keys():
    ...
    pop_results_set_top = []
    pop_results = []
    pop_results_none_zero_likelyhood = []
    pop_results_set_top_none_zero_likelyhood = []
    for rep in range(1, replicates+1):
        file = os.path.join(
            s02_folder, f'SweeD_Report.{pop}.REP{rep}')
        with open(file, 'r') as f:
            set_read = f.read().strip().split("//")
            for i in range(1, len(set_read)):
                set_results = []
                set_results_none_zero_likelyhood = []
                set_i = set_read[i].strip().split("\n")
                for line in set_i[1:]:
                    if line.startswith('#') or line.startswith('Position'):
                        continue
                    else:
                        clr = float(line.split()[1])
                        set_results.append(clr)
                        pop_results.append(clr)
                        if clr > 0:
                            set_results_none_zero_likelyhood.append(clr)
                            pop_results_none_zero_likelyhood.append(clr)
                        elif clr == 0:
                            continue
                        if clr < 0:
                            raise ValueError(wrap79(
                                f'Error: {pop} likelyhood ratio is negative.'))
                set_results.sort(reverse=True)
                set_results_none_zero_likelyhood.sort(reverse=True)
                pop_results_set_top.append(set_results[0])
                pop_results_set_top_none_zero_likelyhood.append(
                    set_results_none_zero_likelyhood[0])
    ...
    # Check the cutoff of none zero likelyhood data if identical
    pop_results_set_top_none_zero_likelyhood.sort(reverse=True)
    cutoff_index = int(len(pop_results_set_top_none_zero_likelyhood)*FPR) - 1
    if cutoff != pop_results_set_top_none_zero_likelyhood[cutoff_index]:
        raise ValueError(wrap79(
            f'Error: {pop} FPR cutoff is not correct.'))

    print(f'{pop} FPR 5% cutoff:=> {cutoff}\n')

    with open(os.path.join(sub_output_dir, f'{pop}.SweeD.FPR005.cutoff.txt'), 'w') as f:
        f.write(f'{pop} CLR cutoff\n')
        f.write(f'{cutoff}\n')
