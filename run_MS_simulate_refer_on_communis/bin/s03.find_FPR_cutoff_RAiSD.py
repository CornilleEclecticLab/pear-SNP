...
# @File     : s03.find_FPR_cutoff_RAiSD.py
# @Author   : NIE Yuqi
# @Email    : nieyuqi.cn@gmail.com
# @Time(CET): 2024-09-16 16:03:25
# @Description:
#    
...
s02_folder = os.path.join(
    output_dir, 's02.generate_positive_selection_detect_RAiSD')
...
for pop in populations.keys():
    ...
    for rep in range(1, replicates+1):
        ...
        file = os.path.join(
            s02_folder, f'RAiSD_Report.{pop}.REP{rep}')
            ...
            for i in range(1,len(set_read)):
                ...
                for line in set_i[1:]:
                    if line.startswith('#') or line == '':
                        ...
                        set_results.append(mu)
                        pop_results.append(mu)
                set_results.sort(reverse=True)
                pop_results_set_top.append(set_results[0])
                n+=1
            print(f'{pop} REP{rep} has {n} sets.')
    ...
    print(f'{pop} results set top number is {len(pop_results_set_top)}.')
    ...
    with open(os.path.join(sub_output_dir, f'{pop}.RAiSD.FPR005.cutoff.txt'), 'w') as f:
        f.write(f'{pop} Mu cutoff\n')
        f.write(f'{cutoff}\n')
