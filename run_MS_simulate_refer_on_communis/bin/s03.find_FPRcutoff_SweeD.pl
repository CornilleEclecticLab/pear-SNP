...
my $folder = "s03.find_FPRcutoff_SweeD";
...
my $s02 = "$workdir/output/s02.generate_positive_selection_detect_SweeD";
...
for my $pop (@pops) {
    ...
    for my $re ( 1 .. 10 ) {
        ...
        open my $inSw, "<", "$s02/SweeD_Report.$pop.REP$re";
        <$inSw>;
        while (<$inSw>) {
            chomp;
            my @block = split /\n/, $_;
    ...
    pop @clrs for ( 1 .. int( @clrs * $cutoff_op ) - 1 );
