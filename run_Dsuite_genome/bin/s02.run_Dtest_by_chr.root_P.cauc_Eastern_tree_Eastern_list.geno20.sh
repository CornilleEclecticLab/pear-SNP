#!/bin/bash
#SBATCH -J s02.run_Dtest_by_chr.root_P.cauc_Eastern_tree_Eastern_list.geno20
#SBATCH -o s02.run_Dtest_by_chr.root_P.cauc_Eastern_tree_Eastern_list.geno20.%J.out
#SBATCH -e s02.run_Dtest_by_chr.root_P.cauc_Eastern_tree_Eastern_list.geno20.%J.err
#SBATCH -c 20
#SBATCH --mem=200G
...
RUN_NAME="root_P.cauc_Eastern_tree_Eastern_list.geno20"

DtriosParallel \
    --run-name $RUN_NAME \
    --keep-intermediate \
    --tree "$WORK_DIR/output/$RUN_NAME/root_cauc_Eastern_tree.nwk" \
    "$WORK_DIR/output/$RUN_NAME/s02.id_species.Eastern.outgroup_cauc.txt" \
...
for mode in tree Dmin BBAA; do
    ruby plot_d.NOGIT.rb \
        "$WORK_DIR/output/$RUN_NAME/DTparallel_s02.id_species.Eastern.outgroup_cauc_${RUN_NAME}_combined_${mode}.txt" \
