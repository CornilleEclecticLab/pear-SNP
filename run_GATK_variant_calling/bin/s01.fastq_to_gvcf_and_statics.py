...
# For bwa-mem2 mapping, 4 CPU and 32G memory is needed. (6.5G bases need 20G memory)
...
# Statistic
samtools depth \\
-a {output_dir}/{sample}{group}.sorted.cram \\
| awk '{{c++;s+=$3}}END{{print s/c}}' > {output_dir}/{sample}{group}.sorted.avg_depth.txt 

samtools depth \\
-a {output_dir}/{sample}{group}.sorted.cram \\
| awk '{{c++; if($3>0) total+=1}}END{{print (total/c)*100}}' > {output_dir}/{sample}{group}.sorted.breadth_coverage.txt 


# GATK MarkDuplicates, .bam
gatk --java-options "-Xmx8g" MarkDuplicates \\
-I {output_dir}/{sample}{group}.sorted.cram \\
-R {ref_genome} \\
...
# Statistic
samtools depth -a {output_dir}/{sample}{group}.sorted.markdu.bam \\
| awk '{{c++;s+=$3}}END{{print s/c}}' > {output_dir}/{sample}{group}.sorted.markdu.avg_depth.txt

samtools depth -a {output_dir}/{sample}{group}.sorted.markdu.bam \\
| awk '{{c++; if($3>0) total+=1}}END{{print (total/c)*100}}' > {output_dir}/{sample}{group}.sorted.markdu.breadth_coverage.txt 
