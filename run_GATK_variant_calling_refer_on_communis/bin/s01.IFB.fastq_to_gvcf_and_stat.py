...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# >>>>>              Here is the start of setting part.                    >>>>>
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
fastq_list = os.path.join(input_dir, group+".input.clean_data_list.txt")
...
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# <<<<<                Here is the end of setting part.                    <<<<<
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
# For bwa-mem2 mapping, 4 CPU and 32G memory is needed. (6.5G uncompressed fastq need 20G memory, but 28 x ref genome size is needed for bwa-mem2 index)
...
\
    ...
    if run == '1':
        command = "sbatch -c 4 -p long --mem=32G -A pear_snp3 "+scripts_dir+"/"+sample_prefix+".sh"
        submit  = os.popen(command, 'r')
        current_job_id  = submit.read().strip().split()[-1]
        depended_job_id = current_job_id
        job_id_txt.write(sample_prefix+'.sh' + '\t' + current_job_id + '\n')
...
'''.format(ref_genome=ref_genome,chr=chr,output_dir=output_dir,sample=sample,group=group,chr_base=chr_base,tmp_dir=tmp_dir)
        ...
        ## Submit this script as a job
        if run == '1':
            command = "sbatch -A pear_snp3 -p long -c 2 --mem=10G --dependency=afterok:"+depended_job_id+' '+scripts_dir+"/"+sample_prefix_chr+".sh"
            submit = os.popen(command, 'r')
            current_job_id  = submit.read().strip().split()[-1]
