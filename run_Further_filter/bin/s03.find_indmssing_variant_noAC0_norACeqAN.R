...
sum_file <- "/shared/ifbstor1/projects/pear_snp3/pear/run_Further_filter/output/s04.pear_Dec2023_variant_noAC0_norACeqAN/pear_Dec2023.Combine_chr.vcf.gz.SNPs_number.txt"
ind_miss_file <- "/shared/ifbstor1/projects/pear_snp3/pear/run_Further_filter/output/s04.pear_Dec2023_variant_noAC0_norACeqAN/pear_Dec2023.Combine_chr.vcf.gz.ind_miss.txt"

nu <- read.table(sum_file)
sum_snp <- nu$V1

miss_df <- read.table(ind_miss_file)

names(miss_df) <- 
  # c("id_vcf", "missing_number","id_individual","species","depth", "dataset")
  c("id_vcf", "missing_number")
  

miss_df$missing_rate <- (miss_df$missing_number / sum_snp) * 100


# Filter species with fewer than 10 individuals, and unclear 12 wild samples
# filtered_miss_df <- miss_df %>%
#   group_by(species) %>%
#   filter(n() >= 12)


# Filter species with missing rate fewer than 40%
