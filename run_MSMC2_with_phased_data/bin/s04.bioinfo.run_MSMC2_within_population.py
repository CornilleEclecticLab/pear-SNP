...
# @Update: v2.0.0 2024-05-28 23:44:45
#   1. Update the script to run MSMC2 with phased data.

# @Update: v3.0.0 2024-08-06 14:30:48
#   1. Special verion on Genobioinfo

# @Update: v3.1.0 2024-09-05 10:14:29
#   1. msmc2ms.py is used to convert the output of MSMC2 as input for ms.
...
unphased = False # If input data is unphased, set True, otherwise False. 
...
for rep in repeats:
    for pop in record_dic[rep].keys():
        ...
        multihetsep_files = [os.path.join(work_dir, "output",
                                          's03.bioinfo.generate_multihetsep',
                                          f'{chr}.{rep}.multihetsep.txt') for chr in chrs]

        multihetsep_files = ' \\\n    '.join(multihetsep_files)

        # Define the output file
        msmc2_output_prefix = os.path.join(output_dir, f'{pop}.{rep}')
...
msmc2_Linux \\
    ...
    -o {msmc2_output_prefix}.msmc2 \\
    {multihetsep_files}

msmc2ms.py \\
    --form snps \\
    {msmc2_output_prefix}.msmc2 \\
> {msmc2_output_prefix}.par
''')
