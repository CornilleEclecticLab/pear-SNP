...
# @Time(CET): 2025-01-22 16:36:03
# @Description:
#    Mask the results from OmegaPlus output.
    ...
    with open(file,'r') as fi:
        for line in fi:
            ...
            if line.startswith('//') or line == '':
                continue
            pos, omega, start, end, vaild = line.split()
            if vaild != '1':
                continue
            start = int(start)-1
            fo_temp.write(
                f'{chr}\t{start}\t{end}\t{omega}\t{pos}\t{vaild}\n')
    fo_temp.close()
    # Write a sub script
...
bedtools intersect \\
    ...
    -wao \\
| awk -v percent_cutoff="$percent_cutoff" '{{if($10 / ($3 - $2) >= percent_cutoff) print $5"\\t"$4"\\t"$2"\\t"$3"\\t"$6}}' \\
> {output_result}
