...
with open(gff_ori) as f:
    for l in f:
        ...
        attributions = splits[8].split(';')
        dic = {i.split('=')[0]:i.split('=')[1] for i in attributions}
        if 'Name' in dic:
            name = dic['Name']
...
with open(os.path.join(sub_output_dir,'positive_selection_summary_table.tsv'),'w') as f:
    header = '\t'.join(['Gene_ID', 'Chr_accession', 'Chr_ID', 'Start', 'End', 'Detected_methods',
                        ...
                        'Detected_populations', 'Gene_name', 'PFAM', 'Gene_description',
                        'GO', 'KEGG_Pathway', 'Reference_articles'])+'\n'
    f.write(header)
    
    specific_pop_files = {}
    for pop in set(pop for pop_methods in pos_genes.values() for pop in pop_methods):
        specific_pop_files[pop] = open(os.path.join(sub_output_dir, f'specific.positive_selection_genes.{pop}.txt'), 'w')
        specific_pop_files[pop].write(header)
    ...
    for gene in gene_list:
        ...
        common_vs_specific = (
            'common' if len(pos_genes[gene]) > 1
            else 'specific' if  len(pos_genes[gene]) ==1
            else ValueError(f'No population detected for gene {gene}') )
        ...
        write_line = f'{gene}\t{chr_accession}\t{chr}\t{start}\t{end}\t{detected_methods}\t{collinearity}\t{common_vs_specific}\t{detected_populations}\t{gene_name}\t{pfam}\t{gene_description}\t{go}\t{kegg_pathways}\t\n'

        f.write(write_line)
        
        if common_vs_specific == 'specific':
            specific_pop_files[pop].write(write_line)
