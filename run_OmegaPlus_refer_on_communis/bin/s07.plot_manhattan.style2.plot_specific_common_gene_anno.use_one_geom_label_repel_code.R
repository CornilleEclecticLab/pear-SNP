...
# Edit the Color_group column
combined_data <- combined_data %>%
  mutate(
    Chr_num = as.numeric(gsub("[^0-9]", "", Chr)),  
    Color_group = case_when(
      Color_group == "Below_Cutoff" & Chr_num %% 2 == 1 ~ "Below_Cutoff_odd",
      Color_group == "Below_Cutoff" & Chr_num %% 2 == 0 ~ "Below_Cutoff_even",
      TRUE ~ Color_group
    )
  )
...
combined_data$Rescaled_times <- combined_data$omegaplus / combined_data$Cutoff

combined_data$Cutoff <- combined_data$Cutoff * combined_data$Rescaled_times
combined_data$Score <- combined_data$Score * combined_data$Rescaled_times
combined_data$raisd <- combined_data$raisd * combined_data$Rescaled_times

# Drop some small data:      log10(score)<0.4
combined_data <- combined_data %>%
  filter(!(Score < 10^0.4 & Score < Cutoff))

head(combined_data)

# Rescale the cutoff
cutoff_df$Rescaled_times <- cutoff_df$omegaplus / cutoff_df$raisd
cutoff_df$raisd <-cutoff_df$raisd * cutoff_df$Rescaled_times
...
if (interactive()) {
  set.seed(123)
  sampled_data <- combined_data[sample(nrow(combined_data), 100000), ]
  combined_data <- sampled_data
  print("INFO: on smapled dataset")
}
...
my_colors_with_grey <- c(my_colors, "Below_Cutoff" = "#AAAAAA",
                         "Below_Cutoff_odd" = "#AAAAAA",
                         "Below_Cutoff_even" = "grey85")
...
# Add gene anno -----------------------------------------------------------
# Done plot by population
# DEV add gene annotation

## /!\ TEST data
# gene_annotations <- data.frame(
#   Gene = c("GeneA", "GeneB", "GeneC", "GeneD", "GeneE"),
#   Chromosome = c("Chr1", "Chr2", "Chr3", "Chr4", "Chr5"),
#   Position = c(105000, 230000, 450000, 720000, 910000),
#   Score = c(4.5, 5.2, 3.9, 3.9, 3.9)  # Example scores
# )

## Read gene annotation
gene_annotations <- read.table("../output/s08.summary_table/manually_interest_common_pop.positive_selection_genes_summary.comm_Dessert.comm_Perry.txt", 
                               sep='\t', header=TRUE) %>% 
  select(Gene_ID, Chr_ID, Start, End, Common_vs_specific_in_population, Ath_symbol) %>%
  filter(!(Ath_symbol %in% c("-", "NA"))) %>%
  na.omit() %>%
  rename(Symbol = Ath_symbol)
print(gene_annotations)


# Merge combined_data with gene_annotations based on matching Chr
combined_with_common_genes <- combined_data %>%
  filter(Score>=Cutoff) %>% 
  left_join(gene_annotations, by = c("Chr" = "Chr_ID"), relationship = "many-to-many") %>%
  filter(is.na(Start.x) | (Start.x <= End.y & End.x >= Start.y)) %>%
  select(-End.y, -Start.y) %>%          # Remove End.x and Start.y
  rename(Start = Start.x, End = End.x) %>%
  group_by(Gene_ID, Population) %>%
  filter(Score == max(Score)) %>%  # Keep the row with the highest Score for each Gene_ID and Population
  ungroup()  # Ungroup after filtering to return to a regular data frame


# View the resulting table
head(combined_with_common_genes)



# Load population specific gene annotation
specific_annotations <- read.table("../output/s08.summary_table/manually_specific.positive_selection_genes.summary.txt",
                                   sep='\t', header=TRUE, quote = "")%>% 
  select(Gene_ID, Chr_ID, Start, End, Common_vs_specific_in_population, Detected_populations, Ath_symbol) %>%
  filter(!(Ath_symbol %in% c("-", "NA"))) %>%
  na.omit() %>%
  rename(Symbol = Ath_symbol)

# Merge combined_data with based on matching Chr
combined_with_specific_genes <- combined_data %>%
  filter(Score>=Cutoff) %>% 
  left_join(specific_annotations, by = c("Chr" = "Chr_ID"), relationship = "many-to-many") %>%
  filter(is.na(Start.x) | (Start.x <= End.y & End.x >= Start.y)) %>%
  select(-End.y, -Start.y) %>%          # Remove End.x and Start.y
  rename(Start = Start.x, End = End.x) %>%
  group_by(Gene_ID, Population) %>%
...
head(combined_with_specific_genes)


# Combine the common and specific genes
combined_with_genes <- bind_rows(combined_with_common_genes, combined_with_specific_genes)
combined_with_genes <- combined_with_genes %>%
  mutate(
    label_fill = case_when(
      Common_vs_specific_in_population == "specific" ~ my_colors_with_grey[Population],
      Common_vs_specific_in_population == "common" ~ "white"
    ),
    label_color = case_when(
      Common_vs_specific_in_population == "specific" ~ "white",
      Common_vs_specific_in_population == "common" ~ my_colors_with_grey[Population]
    )
  )

head(combined_with_genes)
...
my_theme <- theme_minimal() +
  theme(
    ...
    axis.title.x = element_blank(),
    axis.title.y = element_text(size = 6),
    axis.text.x = element_text(angle = 0, hjust = 0.5, size = 5, margin = margin(t = 0)),
    axis.text.y = element_text(size = 5),
    legend.title = element_text(size = 6),
    legend.text = element_text(size = 6), # element_blank(), 
...
#### Plot function
plot_manhattan <- function(data, cutoff_data, title = "", gene_annotaion_data) {
  # Separate significant and non-significant data
  data_significant <- data %>% filter(Color_group != "Below_Cutoff")
  data_nonsignificant <- data %>% filter(Color_group == "Below_Cutoff")
  pop_color <- my_colors_with_grey[data_significant$Population[1]]
  
  ggplot() +
    # Below cutoff
    geom_point(
      data = data_nonsignificant,
      aes(x = Cum_pos, y = log10(Score), color =as.factor(Chr), shape = Method),
      alpha = 0.05, size = 0.2
    ) +
    # Over cutoff 
    geom_point(
      data = data_significant,
      aes(x = Cum_pos, y = log10(Score), color = Color_group, shape = Method),
      alpha = .8, size = 0.5
    ) +
    # Define the shapes for scattrers
    scale_shape_manual(values = c("OmegaPlus" = 16, "RAiSD" = 17)  ## Shape for Mathod
    ) +
    # Add cutoff lines
    geom_hline(
      data = cutoff_data,
      aes(yintercept = log10(raisd), color = "grey"),
      linetype = "dashed", alpha = 1, linewidth = 0.2  ## Dashed cutoff for RAiSD
    ) +
    # geom_hline(
    #   data = cutoff_data,
    #   aes(yintercept = log10(omegaplus), color = "grey"),
    #   linetype = "solid", alpha = 1, linewidth = 0.2  ## Solid cutoff for OmegaPlus
    # ) +
    # Add cutoff line annotations
    # annotate("text", 
    #          x = max(data$Cum_pos) * 0.99, 
    #          y = log10(cutoff_data$raisd[1]), 
    #          label = "Mu cutoff", 
    #          hjust = 1, vjust = -0.5, 
    #          size = 2, color = pop_color
    # ) +
    # annotate("text", 
    #          x = max(data$Cum_pos) * 0.99, 
    #          y = log10(cutoff_data$omegaplus[1]), 
    #          label = "Omega cutoff", 
    #          hjust = 1, vjust = -0.5, 
    #          size = 2, color = pop_color
    # ) +
    ...
    # geom_text_repel(
    geom_label_repel(
      data = gene_annotaion_data, 
      aes(x = Cum_pos, y = log10(Score), label = Symbol), 
      fill = gene_annotaion_data$label_fill,
      color = gene_annotaion_data$label_color,
      #color = pop_color, 
      size = 0.5*.pt,
      alpha = 0.7,
      force = 0.1,
      segment.color = pop_color,
      min.segment.length = 0.05, segment.size = 0.2,
      label.padding = 0.15,
      box.padding = 0.1,    
      point.padding = 0.1,  
      max.iter = 3000,
    ...
    # The chromosome boundary line
    # geom_vline(
    #   xintercept = unique(c(chr_label$start, chr_label$end)), 
    #   color = "grey", linetype = "solid", alpha = 0.3
    # ) +
    # Scales and theme
    scale_x_continuous(
      # name = "Chromosome", 
    ...
    scale_y_continuous(
      # name = "log10(Omega or mu)",
      name = bquote("\u25CF"~~log[10]~"(Omega)"),     # ●
      sec.axis = dup_axis(name = bquote("\u25B2"~~log[10]~"(Rescaled Mu)")),  # ▲
...
cultivar_p1 <- "comm_Dessert"
cultivar_p2 <- "comm_Perry"


sub_combined_data_p1 <- combined_data %>% 
  filter(Population == cultivar_p1)

sub_cutoff_p1 <- cutoff_df %>% 
  filter(Population == cultivar_p1)

sub_combined_with_gene_p1 <- combined_with_genes %>% 
  filter(Population == cultivar_p1)

sub_combined_data_p2 <- combined_data %>% 
  filter(Population == cultivar_p2)
sub_cutoff_p2 <- cutoff_df %>% 
  filter(Population == cultivar_p2)
sub_combined_with_gene_p2 <- combined_with_genes %>% 
  filter(Population == cultivar_p2)


## Plot
print("Start plotting...")
p1 <- plot_manhattan(sub_combined_data_p1, sub_cutoff_p1, cultivar_p1, sub_combined_with_gene_p1)
p1

p2 <- plot_manhattan(sub_combined_data_p2, sub_cutoff_p2, cultivar_p2, sub_combined_with_gene_p2)
p2
combined_plot <- plot_grid(p1, p2, ncol = 1)
# combined_plot

ggsave("s07.plot_Manhattan.style2/combined_plot.pdf", device = cairo_pdf, plot = combined_plot, width = 130, height = 60, units = "mm")
ggsave("s07.plot_Manhattan.style2/combined_plot.png", plot = combined_plot, width = 130, height = 60, units = "mm", dpi = 1200)

ggsave("s07.plot_Manhattan.style2/combined_plot.A4.pdf", device = cairo_pdf, plot = combined_plot, width = 297, height = 210, units = "mm")
ggsave("s07.plot_Manhattan.style2/combined_plot.A4.png", plot = combined_plot, width = 297, height = 210, units = "mm", dpi = 1200)

ggsave("s07.plot_Manhattan.style2/combined_plot.long.pdf", device = cairo_pdf, plot = combined_plot, width = 900, height = 60, units = "mm")
ggsave("s07.plot_Manhattan.style2/combined_plot.long.png", plot = combined_plot, width = 900, height = 60, units = "mm", dpi = 1200)
