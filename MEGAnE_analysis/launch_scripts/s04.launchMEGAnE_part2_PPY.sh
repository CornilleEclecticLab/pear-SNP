#!/bin/bash
#SBATCH --job-name=PPY_MEGAnE_part2_admixed
#SBATCH --output=../logs/MEGAnE_part2_PPY_admixed.out
#SBATCH --error=../logs/MEGAnE_part2_PCPY_admixed.err
...
ROOT="../data/PPY/CRAM/eastern_pears_ref_PPY_admix"
...
find "$ROOT" -type f -name '*.cram' -print0 | while IFS= read -r -d '' cram; do
  ...
  #outdir="../output/PPY/MEGAnE_result/$SAMPLE"
  outdir="../output/PPY/MEGAnE_result_result_admixed/$SAMPLE"
  ...
  singularity exec "${sif}" call_genotype \
    ...
    -fadb ../data/PPY/assembly/GWHBAOS00000000.genome.fasta.reformat \
    -mk ../output/PPY/megane_kmer_set/reference_PPY_genome.mk \
    -outdir "$outdir" \
    -p 128 \
    -rep ../data/PPY/TE_annotation_URGI/PPY3_TEannotGr2_refTEs.fa.RMformat \
    -repout ../data/PPY/TE_annotation_URGI/PPY_TE_annotation.gff3.renameChr.out \
    -repremove ../data/PPY/non_ME_rep.txt \
    -pA_ME ../data/PPY/ME_with_pA.txt \
    -mainchr ../data/PPY/main_chr_list.txt \
    -no_sex_chr
done
