...
version = "2.0.0" # Pysam version, deprecated as it's slow.
...
class VCFParser:
    ...
    def parse_gt(self, gt_tuple):
        if gt_tuple is None or any(allele is None for allele in gt_tuple):
            if gt_tuple == (None, None):
                return None
            else:
                raise ValueError(f"Invalid genotype with partial missing data: {gt_tuple}")

        a1, a2 = gt_tuple
        sum1_m = (1 if a1 == 1 else 0) + (1 if a2 == 1 else 0)
        sum0_m = (1 if a1 == 0 else 0) + (1 if a2 == 0 else 0)
        ...
        if sum1_m == 2:
            ...
            raise ValueError(f"Invalid genotype format: {gt_tuple}")
    ...
    def parse_vcf(self, vcf_path: str) -> dict:
        ...
        for record in vcf_in:
            chrom = record.chrom
            pos = str(record.pos)
            if chrom not in self.sift_annotations:
                raise ValueError(f"Chromosome {chrom} not found in SIFT4G annotations.")
            
            if pos not in self.sift_annotations[chrom]:
                continue
            ...
            # Check for ref/alt mismatch
            if record.ref != ref_anno or ','.join(record.alts) != alt_anno:
                raise ValueError(
                f'Variant {chrom}:{pos} has different ref/alt in VCF and SIFT4G annotation: '
                f'VCF({record.ref}/{record.alts}) vs SIFT4G({sift_annotations[chrom][pos]["ref"]}/{sift_annotations[chrom][pos]["alt"]})')

            for sample_id in record.samples:
                gt_tuple = record.samples[sample_id].get('GT')
                gt_label = self.parse_gt(gt_tuple)
                if gt_label is None:
                    continue
...
def read_individual_pop_map(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            pop_individual.setdefault(pop, []).append(ind)
...
def process_vcf_files(ref_genome, sift_annotations):
    ...
    parser = VCFParser(sift_annotations)
    n_vcf = 0
    for vcf_file in vcf_file_list:
        if not vcf_file.endswith('.vcf.gz'):
            continue
        pop_vcf = os.path.basename(vcf_file).split('.')[-3]   # VCF filename format: *.chr.pop.vcf
        ...
        if not processed_vcf_dic[chr_vcf][pop_vcf]:
            ...
            s_variant= parser.parse_vcf(vcf_file)
