...
#SBATCH -J s02.annotate_vcf_against_sift4g_db_PPY.sh
#SBATCH -o s02.log/s02.annotate_vcf_against_sift4g_db_PPY.sh.%J.out
#SBATCH -e s02.log/s02.annotate_vcf_against_sift4g_db_PPY.sh.%J.err
#SBATCH -c 17
#SBATCH --mem=16G

# @File     :   s02.annotate_vcf_against_sift4g_db_PPY.sh
# @Version  :   1.0.0
# @Author   :   NIE Yuqi
...
while read -r CHR; do
  ...
  # Use bcftools view to fresh the vcf AN, AC, AF fields
  bcftools view \
  ...
  awk '$8=="CDS"' $SIFT_XLS | awk '$13!="NA"' >$SIFT_DIR/$ORG_VERSION.$CHR.cds.tsv
  ...
  cut -f 1,2 $SIFT_DIR/$ORG_VERSION.$CHR.cds.tsv > $SIFT_DIR/$ORG_VERSION.$CHR.cds.pos.txt
  ) &
done < "$CHR_LIST"
