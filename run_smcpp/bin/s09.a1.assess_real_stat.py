...
# @File     : s09.a1.assess_real_sata.py
# @Author   : NIE Yuqi
    ...
    for header in vcf_header:
        for line in header:
            if line.startswith("##contig"):
                contig_length = int(line.split('length=')[-1].replace('>', '').split(',')[0]) # Only one contig in the simulation vcf file
...
def main():
    ...
    glob_vcf = glob.glob(args.folder + '/*Dessert.vcf')
