...
for repeat in range(1, 11): # 1-10
    ...
    for pop in populations:
        ...
        output_file_base = os.path.join(output_dir, f"scrm.{pop}.REP{repeat}")
        sub_script = os.path.join(sub_script_dir, sub_script_base+'.sh')
        if os.path.exists(sub_script):
            print(wrap79(f"File {(sub_script)} already exists. Exiting."))
            exit()              
...
source {bin_dir}/s00.load_scrm.sh

scrm  {len(populations[pop])*2} \\
    100 \\
    -seeds {seed0} {seed1} {seed2} \\
    {par.replace(" 1000000 -p", " 540000000 -p")} \\
