...
# @File     : s01.get_phased_vcf_by_pop_by_chr.py
# @Author   : NIE Yuqi
# @Email    : nieyuqi.cn@gmail.com
# @Time(CET): 2024-11-26 11:49:08
# @Description:
#
...
import os
import warnings
from s00_config import individual_pop_map_filename, chromosomes_list_filename, phased_vcf_list_filename
...
# Read the phased VCF list by chromosome
with open(os.path.join(input_dir, phased_vcf_list_filename), 'r') as file:
    phased_vcf_list = [os.path.realpath(i)
                        for i in file.read().strip().split()]
phased_chr_vcf_dic = {os.path.basename(path).split(
    ".")[0]: path for path in phased_vcf_list}
# Variation vcf chr list
for chr in chromosomes:
    if chr not in phased_chr_vcf_dic:
        print(phased_chr_vcf_dic)
        raise ValueError(f'Chromosome {chr} is not in the phased vcf list')
for chr in phased_chr_vcf_dic:
...
for pop_name, ids in pop_individual.items():
    ...
    for chr in chromosomes:
        ...
        nonadmix_vcf = os.path.join(
            output_dir, f'phased.{chr_base}.{pop_name}.vcf')
        masked_vcf = os.path.join(
            output_dir, f'phased.{chr_base}.{pop_name}.masked.vcf')
        ...
        with open(os.path.join(sub_script_dir, sub_script_basename+'.sh'), 'w') as fo:
            ...
            fo.write(run_bcftools.format(bin_dir=bin_dir,
                                         ...
                                         vcf=phased_chr_vcf_dic[chr_base],
                                         input_dir=input_dir,
