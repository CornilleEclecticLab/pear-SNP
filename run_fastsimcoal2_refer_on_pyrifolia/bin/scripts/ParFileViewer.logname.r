...
if (genTime==1) ylabel="log10(time (gen))" else ylabel="log10(time (years))"

plot(x=1:numSamples, type="n", xlab="", ylab="", xlim=c(-0.5, numSamples+1.5), cex.main=0.8,
     ylim=c(0, log10(yTimeLimit)), main=title, xaxt = 'n', cex.axis=0.9, cex.lab=0.9, las=2)
...
for (i in 1:numDemes) {
  for (j in 2:numPopSizes[i]) if (popSizes[j,i]>0) {
    ...
    vec <- c(bottomLeftY, topLeftY, topRightY, bottomRightY)
    vec_adjusted <- ifelse(vec == 0, 1, vec)
    
    polygon(c(bottomLeftX, topLeftX, topRightX, bottomRightX), log10(vec_adjusted), 
            col=popCol)
...
curMigMatNum=0
curvature=0.0075*log10(last.he.time)

if (numMigMat) {
  if (numMigMat==1) time2DrawArrows=log10(yTimeLimit)/2 else {
    time2DrawArrows=log10(timeMigMatChanges[1])/2
  }
...
vec <- ifelse(timeInstBot == 0, 1, timeInstBot)
timeInstBot_log <- log10(vec)
...
if (max(numInstBot)>0) {
  ...
  for (i in 1:numDemes) if (numInstBot[i]) {
    for (j in 1:numInstBot[i]) {
      ...
      draw.circle(i, timeInstBot_log[j,i], radius = curRadius*(popSizes[j,i]-sizeInstBot[j,i])/popSizes[j,i], col=instbotColor, border=instbotColor)
      #Add inst bot intensity on top right
      xpos=i+curRadius
      if (instBotAbsSize) botText=sizeInstBot[j,i] else botText=1.0/sizeInstBot[j,i]
      text( i, timeInstBot_log[j,i]*(1.005), labels=format(botText,digits=1), cex=timeProp, col=textInstbotColor, pos=3)
      curPopSize=getPopSize(timeInstBot_log[j,i], i, popSizes, popSizeTimeChange, numPopSizes)
      curPopRadius=interpolRadius(curPopSize, minPopSize, maxPopSize, minRadius, maxRadius, drawLogPopSize)
      text(i+curPopRadius, timeInstBot_log[j,i], labels=format(timeInstBot_log[j,i],digits=1), cex=timeProp, col=textInstbotColor, pos=4)
...
vec <- ifelse(timeMigEvent == 0, 1, timeMigEvent)
timeMigEvent_log <- log10(vec)

for (i in 1:numDemes) if (numMigEvents[i]) {
  for (j in 1:numMigEvents[i]) {
    curRadius=interpolRadius(getPopSize(timeMigEvent_log[j,i], i, popSizes, popSizeTimeChange, numPopSizes), minPopSize, maxPopSize, minRadius, maxRadius, drawLogPopSize)
    #Draw connecting arrows from source to sink      
    if (sourceMigEvent[j,i] > i) target = i+curRadius else target = i-curRadius
    segments(sourceMigEvent[j,i], timeMigEvent_log[j,i], target, timeMigEvent_log[j,i], col=admixCol, lty=2)
    ...
    fullHeadArrow(posArrowX, timeMigEvent_log[j,i], target, timeMigEvent_log[j,i], length=0.15, angle=20, color=admixCol)
    #Redraw time with the right color
    text(timesPos+slide, timeMigEvent_log[j,i], labels=timeMigEvent[j,i], cex=timeProp, col=admixCol)
    slide=-slide
    #Add admixture level on top of arrow
    text(posTextX, timeMigEvent_log[j,i], labels=format(sizeMigr[j,i], digits=3), cex=timeProp, col=admixCol, pos=3)
  }
}

#--- Draw all gene flow arrows (11.03.21)

vec <- ifelse(timeMigMatChanges == 0, 1, timeMigMatChanges)
timeMigMatChanges_log <- log10(vec)
yTimeLimit_log <- log10(yTimeLimit)

if (numMigmatChanges) {
  for (i in 1:numMigmatChanges) {
    if (i<numMigmatChanges) time2DrawArrows=(timeMigMatChanges_log[i+1]+timeMigMatChanges_log[i])/2 else {
      time2DrawArrows=(timeMigMatChanges_log[i]+yTimeLimit_log)/2 
    }
    ...
    segments(migrMatPos-migMatLineLength/2, timeMigMatChanges_log[i], 
             migrMatPos+migMatLineLength/2, timeMigMatChanges_log[i], lty=3, col=migrMatCol)
  }
}
...
if (numHistEvents) {
  for (i in 1:numHistEvents) {
    ...
    if (he.migr>=1 & he.sink!=he.source) { #This is a population fusion
      ...
      segments(he.source, log10(he.time), target, log10(he.time), col = popFusionColor, lty=1) 
      #Redraw time with the right color
      text(timesPos+slide, log10(he.time), labels=he.time, cex=timeProp, col=popFusionColor)
      slide=-slide
    }   
